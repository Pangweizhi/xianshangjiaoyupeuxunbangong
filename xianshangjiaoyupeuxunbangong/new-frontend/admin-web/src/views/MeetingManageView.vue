<template>
  ...
  <el-dialog v-model="dialogVisible" :title="form.id ? '编辑会议' : '新增会议'" width="680px">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="会议类型" prop="kaihuitongzhiTypes">
        <el-select v-model="form.kaihuitongzhiTypes">
...
import { fetchMeetings } from "@/api/dashboard";
...
const createForm = () => ({
  ...
  kaihuitongzhiName: "",
  kaihuitongzhiTypes: undefined as number | undefined,
  kaihuitongzhiContent: "",
  kaihuitongzhiDelete: 1
});
const form = reactive(createForm());

const rules: FormRules = {
  kaihuitongzhiName: [{ required: true, message: "请输入会议标题", trigger: "blur" }],
  kaihuitongzhiTypes: [{ required: true, message: "请选择会议类型", trigger: "change" }],
  kaihuitongzhiContent: [{ required: true, message: "请输入会议详情", trigger: "blur" }]
};

async function loadItems() {
  loading.value = true;
  try {
    const page = await fetchMeetings({ page: pagination.page, limit: pagination.limit, kaihuitongzhiName: keyword.value || undefined });
...
async function openCreate() {
  resetForm();
  typeOptions.value = await fetchDictionaryOptions("kaihuitongzhi_types");
...
async function openEdit(id: number) {
  resetForm();
  typeOptions.value = await fetchDictionaryOptions("kaihuitongzhi_types");
  Object.assign(form, await fetchEntityDetail("kaihuitongzhi", id));
  dialogVisible.value = true;
}
...
async function submitForm() {
  ...
  saving.value = true;
  try {
    await saveEntity("kaihuitongzhi", form as unknown as Record<string, unknown>);
...
async function removeItem(id: number) {
  await deleteEntities("kaihuitongzhi", [id]);
